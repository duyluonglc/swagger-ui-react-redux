...
import { memoizedSampleFromSchema, memoizedCreateXMLExample } from './fn'
...
export function objectify (thing) {
  ...
  if (isImmutable(thing)) { return thing.toObject() }
  return thing
}
...
export function fromJSOrdered (js) {
  if (isImmutable(js)) { return js } // Can't do much here
...
export function objReduce (obj, fn) {
  return Object.keys(obj).reduce((newObj, key) => {
    ...
    if (res && typeof res === 'object') { assign(newObj, res) }
    return newObj
  }, {})
}
...
export function defaultStatusCode (responses) {
  return responses[DEFAULT_REPONSE_KEY] ? DEFAULT_REPONSE_KEY : _(responses).keys().filter(key => (key + '')[0] === '2').sort().first()
}
...
export function highlight (el) {
  ...
  var reset = function (el) {
    ...
    var token = el.innerHTML = ''  // (and cleaning the node)
