...
import { systemThunkMiddleware, isFn, objMap, objReduce, isObject, isArray, isFunc } from './utils'
...
export default class Store {
  ...
  getType (name) {
    ...
    return objReduce(this.system.statePlugins, (val, namespace) => {
      let thing = val[name]
      if (thing) { return {[namespace + upName]: thing} }
    })
  }
  ...
  getActions () {
    ...
    return objMap(actionHolders, (actions) => {
      return objReduce(actions, (action, actionName) => {
        if (isFn(action)) { return {[actionName]: action} }
      })
    })
  }
  ...
  getComponents (component) {
    if (typeof component !== 'undefined') { return this.system.components[component] }
    ...
    return objMap(this.getSelectors(), (obj, key) => {
      ...
      return objMap(obj, (fn) => {
        return (...args) => {
          ...
          if (typeof (res) === 'function') { res = res(getSystem()) }
...
function combinePlugins (plugins, toolbox) {
  if (isObject(plugins) && !isArray(plugins)) { return plugins }

  if (isFunc(plugins)) { return combinePlugins(plugins(toolbox), toolbox) }
